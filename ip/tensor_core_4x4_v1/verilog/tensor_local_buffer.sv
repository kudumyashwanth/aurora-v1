...
logic [$clog2(BUFFER_DEPTH)-1:0] porta_word_addr;
logic [$clog2(BUFFER_DEPTH)-1:0] portb_word_addr;

// Divide byte address by 16 (128-bit words), then index the buffer depth.
assign porta_word_addr = porta_addr[$clog2(BUFFER_DEPTH)-1+4:4];
assign portb_word_addr = portb_addr[$clog2(BUFFER_DEPTH)-1+4:4];
