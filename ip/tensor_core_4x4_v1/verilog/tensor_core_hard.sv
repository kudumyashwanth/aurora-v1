...
#(
    ...
    parameter SIZE       = 4     // 4x4 = 16-MAC tile (routes+signs-off clean at normal density;
                                 // tile x4/x16 for higher throughput, or SIZE up on commercial P&R)
)
(
    input  logic clk,
    input  logic rst_n,
    ...
    // 8x8 tile reads only words 0-1 (A@0x00, B@0x10); a 16-deep flop buffer is
    // ample and removes the 4 SRAM macros (73k routing blockages = the tc2-tc11
    // global-route congestion wall). Real SRAM returns at full 16x16/4-tile scale.
    tensor_local_buffer #(
        .DATA_WIDTH(128),
        .ADDR_WIDTH(16),
        .BUFFER_DEPTH(16)
    ...
    localparam int WPM = (SIZE + EPW - 1) / EPW;    // ceil: buffer words per matrix (>=1 even if SIZE<EPW)
    ...
    always_ff @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            if (rd_pend) begin
                if (rd_idx < WPM) begin
                    // guard e against SIZE so SIZE<EPW (e.g. 4x4: 4 elems in an 8-elem word) is safe
                    for (int e = 0; e < EPW; e++)
